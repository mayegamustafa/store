...
export function PopularProducts() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => productsApi.list({ sort: 'sales', limit: 10 }).then((r: any) => r.data?.data ?? r.data ?? []),
  });
        ...
        return (
          <div key={p.id} className="product-card relative">
            <Link href={`/products/${p.slug}`}>
              ...
              {addItem && (
              <button onClick={() => addItem({ productId: p.id, name: p.name, price: base, image: p.images?.[0], quantity: 1 })}
